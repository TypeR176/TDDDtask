...
# 模型单例对象，避免重复初始化
_bge_m3_ef = None
_bge_m3_init_failed = False

def get_bge_m3_ef():
    global _bge_m3_ef, _bge_m3_init_failed
    if _bge_m3_ef is not None:
        logger.debug("BGE-M3模型单例已存在，直接返回实例")
        return _bge_m3_ef
    if _bge_m3_init_failed:
        raise RuntimeError("BGE-M3模型初始化已失败，不再重试")

    # 从环境变量加载配置，无配置则使用默认值
    # 本地有可以使用本地地址！ 没有使用 "BAAI/bge-m3" 会自动下载！ 如果云端部署也可以使用url地址！
    model_name = embedding_config.bge_m3_path or "BAAI/bge-m3"
    device = embedding_config.bge_device or "gpu"
